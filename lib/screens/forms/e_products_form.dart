...
class _EProductsStepState extends State<EProductsStep>
  ...
  Widget _internetBankingCheckBox() {
    return StreamBuilder<bool?>(
      stream: accountFormBloc!.isInternetBanking,
      builder: (context, snapshot) {
        return CheckboxListTile(
          onChanged: accountFormBloc!.changeInternetBanking,
          title: new Text('Internet Banking'),
          controlAffinity: ListTileControlAffinity.leading,
          activeColor: Colors.red,
          dense: true,
          value: snapshot.hasData ? snapshot.data : false,
      ...
      builder: (context, boolSnapshot) {
        if (boolSnapshot.hasData && boolSnapshot.data!) {
          return StreamBuilder<String?>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (_requestingBranchController!.text !=
      ...
      builder: (context, boolSnapshot) {
        if (boolSnapshot.hasData && boolSnapshot.data!) {
          return StreamBuilder<String?>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (_destinationBranchController!.text !=
                    snapshot.data.toString()) {
      ...
      builder: (context, boolSnapshot) {
        if (boolSnapshot.hasData && boolSnapshot.data!) {
          return StreamBuilder<String?>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (_preferredNameOnCardController!.text !=
                    snapshot.data.toString()) {
  ...
  Widget selectAllButton() => Align(
        ...
        child: TextButton(
          child: Text(
            'Select All',
            style: TextStyle(fontSize: 14.0, color: ZxplorePrimaryColor),
          ),
          style: ButtonStyle(),
          onPressed: () {
