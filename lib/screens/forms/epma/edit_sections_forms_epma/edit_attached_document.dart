...
class EditAttachedDocument extends ConsumerStatefulWidget {
 ...
 final GetDocumentAttachedToEditResponse? data;
  ...
  @override
  ConsumerState<EditAttachedDocument> createState() => _EditAttachedDocumentState();
}
    ...
    if (fileSizeInMB.toInt() <= 10) {
      if (selectedFile != null) {
        final success = await ref
            .watch(fileUploadControllerProvider.notifier)
            .uploadFile(
              file: selectedFile,
        ...
        if (success) {
          ...
          zXFlushBar(context, "Document Uploaded Failed . Try again");
        }
      }
    } else {
      if (mounted) {
