...
class _ViewForeignAccountState extends ConsumerState<ViewForeignAccount> {
  ...
  Widget build(BuildContext context) {
    ...
    return ZxploreProgress(
      inAsyncCall:
          ref.watch(viewForeignAaccountsControllerProvider).isLoading ||
...
class Item extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return FoldableItem(
      userWrapper: true,
      name:
          'Values: From Business Income-${data?.fundSourceBusinessIncome ?? ''},From Salary-${data?.fundSourceSalary ?? ''},Other sources-${data?.fundSourceOther ?? ''}...',
      number:
          'More settings:On-Shore EUR ${data?.onShoreEur ?? ''} , On-Shore GBP${data?.onShoreGbp ?? ''},On-Shore USD ${data?.onShoreUsd ?? ''},Off-Shore Eur ${data?.offShoreEur ?? ''} ,Off-Shore USD-${data?.offShoreUsd ?? ''},Off-Shore Gbp-${data?.offShoreGbp ?? ''}, Off-Shore Usd-${data?.offShoreUsd ?? ''}...',
      requestId: data?.reqId,
      subRequestId: data?.foreignAccountId,
      onTapEdit: () {
        // to navigate to edit this section
        ref.read(viewForeignAaccountsControllerProvider.notifier).getEditData(
              context,
              RequestId: data?.reqId ?? '',
            );
      },
      ...
      onTapDelete: () {
        ref
            .read(viewForeignAaccountsControllerProvider.notifier)
            .deleteForeignAccount(context,
                RequestId: data?.reqId ?? '',
                delData: DeleteForeignAccount(
